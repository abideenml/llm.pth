...
Microsoft's Phi-3 model with:
- Custom RoPE implementation (traditional style)
...
from llm.core import model_summary
...
class RoPE(nn.Module):
    """Rotary Position Embedding (Phi-style implementation)."""
...
class PhiMHA(nn.Module):
    """Phi Multi-Head Attention with RoPE."""

    def __init__(
        self, layer_idx: int, dim: int, num_heads: int, rotary_dim: float
    ) -> None:
        super().__init__()
        self.dim = dim
...
class Block(nn.Module):
    """Phi transformer block with parallel attention and MLP."""

    def __init__(self, config: PhiConfig, block_idx: int | None = None) -> None:
        super().__init__()
...
class Phi(nn.Module):
    ...
    def from_pretrained(name: str) -> nn.Module:
        """Load pretrained Phi model from safetensors."""
        config = PhiConfig()
        model = Phi(config)
