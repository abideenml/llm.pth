...
class SwiGLU(torch.nn.Module):
    ...
    def forward(self, x):
        return self.w2(torch.nn.functionals.silu(self.w1(x)) * self.w3(x))
...
class Attention(torch.nn.Module):
    def __init__(self, model_args: Gemma2Config, layer_idx: Optional[int] = None):
        ...
        self.num_kv_heads = (model_args.num_heads if model_args.num_kv_heads == 0 else model_args.num_kv_heads)
        ...
        self.sliding_window = model_args.sliding_window if not bool(layer_idx % 2) else None
...
class Block(torch.nn.Module):
    def __init__(self, model_args: Gemma2Config, layer_idx: Optional[int] = None ):
        ...
        self.attn = Attention(model_args, layer_idx)
        self.ff = SwiGLU(
...
class Gemma2(torch.nn.Module):
    def __init__(self, model_args: Gemma2Config, *args, **kwargs) -> None:
        ...
        self.layers = torch.nn.ModuleList([Block(model_args, layer_idx) for layer_idx in range(model_args.num_layers)])
