...
Extends DeepSeek V2 with:
- Dynamic Sparse Attention (DSA) with learned indexing
- Lightning Indexer for efficient sparse attention
"""
...
class LightningIndexer(nn.Module):
    """Learned indexer for Dynamic Sparse Attention (DSA)."""

    def __init__(self, d_model: int, d_index: int, num_heads: int):
        super().__init__()
        self.num_heads = num_heads
        self.q_proj = nn.Linear(d_model, num_heads * d_index, bias=False)
        self.k_proj = nn.Linear(d_model, d_index, bias=False)
        self.w_proj = nn.Linear(d_model, num_heads, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            index_scores: Attention index scores (B, T, T)
        """
        B, T, _ = x.shape
...
class DSAAttention(nn.Module):
    """DeepSeek V3.2 Attention with Dynamic Sparse Attention (DSA)."""
    ...
    def __init__(self, model_args: Deepseekv32Config):
        ...
        self.indexer = LightningIndexer(
            d_model, model_args.dsa_index_dim, model_args.dsa_index_heads
        )
    ...
    def forward(
        ...
        topk = self.dsa_topk
        _, topk_idx = torch.topk(index_scores, k=topk, dim=-1)

        # Sparse attention computation
        key_states_permuted = key_states.permute(0, 2, 1, 3)
        value_states_permuted = value_states.permute(0, 2, 1, 3)
        batch_idx = torch.arange(batch, device=x.device)[:, None, None]
        k_sparse = key_states_permuted[batch_idx, topk_idx]
        v_sparse = value_states_permuted[batch_idx, topk_idx]

        k_sparse = k_sparse.permute(0, 3, 1, 2, 4)
        v_sparse = v_sparse.permute(0, 3, 1, 2, 4)

        scores = (query_states.unsqueeze(-2) * k_sparse).sum(dim=-1)
        scores /= math.sqrt(self.head_dim)
        attn = torch.softmax(scores, dim=-1)
        attn = self.attn_dropout(attn)

        output = (attn.unsqueeze(-1) * v_sparse).sum(dim=-2)
...
class Block(nn.Module):
    def __init__(self, model_args: Deepseekv32Config):
        super().__init__()
        self.attn = DSAAttention(model_args)
        if model_args.moe:
            self.ff = MoE(
                model_args.d_model,
