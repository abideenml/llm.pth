...
class PEER_MOEConfig:
    ...
    bias: int = False
    moe: bool = True
    num_experts: int = 256**2
    # num_experts: int = 1024**2
    k_dim: int = 128
    knn: int = 8  # top k
    # knn: int = 16
    heads: int = 4 # Memory-Heads for each query in PEER
    # heads: int = 8 # Memory-Heads for each query in PEER
    input_dropout: int = 0 # Dropout for PEER
    expert_dim: int = 1 # d_expert, singleton in PEER
...
class Attention(nn.Module):
    def __init__(self, model_args: PEER_MOEConfig):
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, model_args: PEER_MOEConfig):
        ...
        if model_args.moe:
            self.ff = PEER(PEER_MOEConfig, model_args.d_model, model_args.multiple_of * model_args.d_model, model_args.num_experts, model_args.knn)
        else:
            self.ff = SwiGLU(
...
class PEER_MOE(nn.Module):
    def __init__(self, model_args: PEER_MOEConfig, *args, **kwargs) -> None:
