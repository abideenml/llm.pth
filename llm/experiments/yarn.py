...
def precompute_freqs_cis_linear(dim: int, end: int, theta: float = 10000.0):
...
def precompute_freqs_cis_yarn(dim: int, original_max_position_embeddings: int, theta: float = 10000.0, scale: int = 16, beta_fast:int=32, beta_slow:int=1, mscale: float=0.707,  max_position_embeddings: int=2048):
...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # (batch_size * seq_len, dim) @ (dim, num_experts)  = (batch_size * seq_len, num_expert)
        # (token, probability of which expert)
        scores = self.gate(x)
        ...
        # (total_tokens,dim)
        output = torch.empty_like(x, dtype=torch.bfloat16, device=x.device)
...
class Mixtral(nn.Module):
    def __init__(self, model_args: MOEConfig, *args, **kwargs) -> None:
        ...
        self.cis = precompute_freqs_cis_ntk(
